...
to be: tanth, M1, M2, lambda2, lambda3, lambda4, lambda5. We also include in the final file the corresponding values
...
# ---------------------
tanth_min = 1.1487e-9 # (vDelta = 1.0e-9 GeV)
tanth_max = 1.1487e-1 # (vDelta = 1.0e1 GeV)
# ---------------------
M1coeff_min = -30.0
M1coeff_max = 30.0
# ---------------------
...
def sinth(tanth):
    numerator = tanth
    denominator = cmath.sqrt(1 + np.square(tanth))
    return numerator / denominator


def costh(tanth):
    return cmath.sqrt(1 - np.square(sinth(tanth)))


def vDelta(tanth):
    numerator = v * sinth(tanth)
    denominator = 2 * cmath.sqrt(2)
    return numerator / denominator


def vphi(tanth):
    numerator = 2 * cmath.sqrt(2) * vDelta(tanth)
    denominator = tanth
    return numerator / denominator


def m3(tanth, M1, lam5):
    x = M1 / (4 * vDelta(tanth))
    y = lam5 / 2
    return cmath.sqrt(np.square(v) * (x + y))


def m5(tanth, M1, M2, lam3, lam5):
    x = M1 * np.square(vphi(tanth)) / (4 * vDelta(tanth))
    y = 12 * M2 * vDelta(tanth)
    z = (3 / 2) * lam5 * np.square(vphi(tanth))
    w = 8 * lam3 * np.square(vDelta(tanth))
    return cmath.sqrt(x + y + z + w)


def M12sq(tanth, M1, lam2, lam5):
    p = (np.sqrt(3) / 2) * vphi(tanth)
    x = -M1
    y = 4 * vDelta(tanth) * (2 * lam2 - lam5)
    return p * (x + y)


def M22sq(tanth, M1, M2, lam3, lam4):
    x = M1 * np.square(vphi(tanth)) / (4 * vDelta(tanth))
    y = -6 * M2 * vDelta(tanth)
    z = 8 * np.square(vDelta(tanth)) * (lam3 + 3 * lam4)
    return x + y + z


def lam1(tanth, M1, M2, lam2, lam3, lam4, lam5):
    p = 1 / (8 * np.square(vphi(tanth)))
    x = np.square(mh)
    y = np.square(M12sq(tanth, M1, lam2, lam5)) / (M22sq(tanth, M1, M2, lam3, lam4) - np.square(mh))
    return p * (x + y)


def M11sq(tanth, M1, M2, lam2, lam3, lam4, lam5):
    return 8 * lam1(tanth, M1, M2, lam2, lam3, lam4, lam5) * np.square(vphi(tanth))


def mu2sq(tanth, M1, M2, lam2, lam3, lam4, lam5):
    x = -4 * lam1(tanth, M1, M2, lam2, lam3, lam4, lam5) * np.square(vphi(tanth))
    y = -3 * (2 * lam2 - lam5) * np.square(vDelta(tanth))
    z = (3 / 2) * M1 * vDelta(tanth)
    return x + y + z


def mu3sq(tanth, M1, M2, lam2, lam3, lam4, lam5):
    x = -(2 * lam2 - lam5) * np.square(vphi(tanth))
    y = -4 * (lam3 + 3 * lam4) * np.square(vDelta(tanth))
    z = M1 * np.square(vphi(tanth)) / (4 * vDelta(tanth))
    w = 6 * M2 * vDelta(tanth)
    return x + y + z + w


def mH(tanth, M1, M2, lam2, lam3, lam4, lam5):
    x = M11sq(tanth, M1, M2, lam2, lam3, lam4, lam5) + M22sq(tanth, M1, M2, lam3, lam4)
    y = np.square(mh)
...
def MASS_CSTR_1(tanth, M1, lam5):
    if np.isreal(m3(tanth, M1, lam5)) and np.real(m3(tanth, M1, lam5)) >= 0:
        return True
    else:
        return False


def MASS_CSTR_2(tanth, M1, M2, lam3, lam5):
    if np.isreal(m5(tanth, M1, M2, lam3, lam5)) and np.real(m5(tanth, M1, M2, lam3, lam5)) >= 0:
        return True
    else:
        return False


def MASS_CSTR_3(tanth, M1, M2, lam2, lam3, lam4, lam5):
    if np.isreal(mH(tanth, M1, M2, lam2, lam3, lam4, lam5)) and np.real(mH(tanth, M1, M2, lam2, lam3, lam4, lam5)) >= 0.0 and np.real(mH(tanth, M1, M2, lam2, lam3, lam4, lam5)) <= 712.0:
        return True
    else:
        return False
...
def UNI_CSTR_1(tanth, M1, M2, lam2, lam3, lam4, lam5):
    if np.real(cmath.sqrt(np.square(6 * lam1(tanth, M1, M2, lam2, lam3, lam4, lam5) - 7 * lam3 - 11 * lam4)
                          + 36 * np.square(lam2)) + abs(6 * lam1(tanth, M1, M2, lam2, lam3, lam4, lam5)
...
def UNI_CSTR_2(tanth, M1, M2, lam2, lam3, lam4, lam5):
    if np.real(cmath.sqrt(np.square(2 * lam1(tanth, M1, M2, lam2, lam3, lam4, lam5) + lam3 - 2 * lam4)
                          + np.square(lam5)) + abs(2 * lam1(tanth, M1, M2, lam2, lam3, lam4, lam5)
                                                   - lam3 + 2 * lam4)) < 4 * np.pi:
        return True
    else:
        return False
...
def BOUND_CSTR_1(tanth, M1, M2, lam2, lam3, lam4, lam5):
    if lam1(tanth, M1, M2, lam2, lam3, lam4, lam5) > 0:
        return True
    else:
        return False
...
def BOUND_CSTR_3(tanth, M1, M2, lam2, lam3, lam4, lam5):
    if any(lam2 - omega * lam5 + 2 * cmath.sqrt(lam1(tanth, M1, M2, lam2, lam3, lam4, lam5) * (zeta * lam3 + lam4)) < 0
...
f = open('GM_1_random_points.txt', 'w')
